// src/features/home/components/homeTimeScale.jsx
function HomeTimeScale({ timeSettings, scrollRef, onHostScroll, endAbs }) {
  ...
  for (let abs = startMin; abs <= endAbs; abs += 15) {
    ...
    const min = dayRel % 60;
    const label =
      dayRel === 0 && abs > 0 ? "24:00" : `${String(hour).padStart(2, "0")}:${String(min).padStart(2, "0")}`;
  ...
  return (
    <div className="time-scale" ref={scrollRef} onScroll={onHostScroll}>
      <div className="time-scale-track" style={{ width: `var(--timeline-width)` }}>
        {ticks.map((t) => (
          <div key={t.abs} className={`tick ${t.isHour ? "tick--hour" : "tick--q"} ${t.isMidnight ? "tick--midnight" : ""}`}>
